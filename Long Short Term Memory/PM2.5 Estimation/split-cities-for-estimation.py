...
parser.add_argument('--savepath', type=str, default='cities-estimation', help='save path for csv files')
...
def add_last_measurement(df, targetcol, lastmcol):
    ns, nc = df.shape
...
def remove_extra_cols(df, removecols, targetcol, lastmcol):
    removecols = [i for i in removecols if len(i) > 0]
    cols = list(df)
    for i in [targetcol,lastmcol]+removecols:
        if i in cols:
            cols.remove(i)

    cols.sort()
    df = df.get([targetcol,lastmcol]+cols)
    return df
...
for city in city_list:
    ...
    dfci = add_last_measurement(dfci, args.targetcol, args.lastmcol)
    dfci = remove_extra_cols(dfci, args.removecols.split(','), args.targetcol, args.lastmcol)
